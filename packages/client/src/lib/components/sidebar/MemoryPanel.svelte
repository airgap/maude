<script lang="ts">
  ...
  import type { MemoryCategory, SoulMemoryFileKind } from '@e/shared';
  import { SOUL_MEMORY_FILES } from '@e/shared';
  import { soulMemoryStore } from '$lib/stores/soul-memory.svelte';
  import { primaryPaneStore } from '$lib/stores/primaryPane.svelte';

  type ViewMode = 'files' | 'workspace' | 'skills' | 'rules' | 'soul';
  ...
  // --- Soul Memory ---
  let soulEditingKind = $state<SoulMemoryFileKind | null>(null);
  let soulEditContent = $state('');
  let soulSaving = $state(false);
  let soulPreviewMode = $state(false);

  async function loadSoulMemory() {
    if (settingsStore.workspacePath) {
      await soulMemoryStore.load(settingsStore.workspacePath);
    }
  }

  function startSoulEdit(kind: SoulMemoryFileKind) {
    const file = soulMemoryStore.getFile(kind);
    soulEditingKind = kind;
    soulEditContent = file?.content || '';
    soulPreviewMode = false;
  }

  function cancelSoulEdit() {
    soulEditingKind = null;
    soulEditContent = '';
    soulPreviewMode = false;
  }

  async function saveSoulEdit() {
    if (!soulEditingKind || !settingsStore.workspacePath) return;
    soulSaving = true;
    try {
      await api.soulMemory.saveFile(soulEditingKind, settingsStore.workspacePath, soulEditContent);
      soulEditingKind = null;
      soulEditContent = '';
      soulPreviewMode = false;
      await soulMemoryStore.load(settingsStore.workspacePath);
      uiStore.toast('Memory file saved', 'success');
    } catch {
      uiStore.toast('Failed to save memory file', 'error');
    } finally {
      soulSaving = false;
    }
  }

  async function initSoulFile(kind: SoulMemoryFileKind) {
    if (!settingsStore.workspacePath) return;
    try {
      await api.soulMemory.initFile(kind, settingsStore.workspacePath);
      await soulMemoryStore.load(settingsStore.workspacePath);
      uiStore.toast('Memory file created', 'success');
    } catch {
      uiStore.toast('File already exists or creation failed', 'error');
    }
  }

  async function openSoulFileInEditor(kind: SoulMemoryFileKind) {
    const file = soulMemoryStore.getFile(kind);
    if (file?.exists && file.path) {
      try {
        const res = await api.files.read(file.path);
        if (res.ok) {
          primaryPaneStore.openFileTab(file.path, res.data.content, 'markdown');
        }
      } catch {
        // Fallback: open with the content we already have
        primaryPaneStore.openFileTab(file.path, file.content, 'markdown');
      }
    }
  }
...
<div class="memory-panel">
  ...
  {#if viewMode === 'soul'}
    <div class="soul-memory-view">
      {#if soulEditingKind}
        <!-- Editing a soul memory file -->
        {@const def = SOUL_MEMORY_FILES.find((f) => f.kind === soulEditingKind)}
        <div class="edit-view">
          <div class="edit-header">
            <span class="edit-file truncate">{def?.fileName || soulEditingKind}</span>
            <div class="edit-actions">
              <button class="btn-sm" onclick={() => (soulPreviewMode = !soulPreviewMode)}>
                {soulPreviewMode ? 'Edit' : 'Preview'}
              </button>
              <button class="btn-sm" onclick={cancelSoulEdit}>Cancel</button>
              <button class="btn-sm primary" onclick={saveSoulEdit} disabled={soulSaving}>
          ...
          {#if soulPreviewMode}
            <div class="soul-preview">
              {#each soulEditContent.split('\n') as line}
                {#if line.startsWith('# ')}
                  <h1 class="preview-h1">{line.slice(2)}</h1>
                {:else if line.startsWith('## ')}
                  <h2 class="preview-h2">{line.slice(3)}</h2>
                {:else if line.startsWith('### ')}
                  <h3 class="preview-h3">{line.slice(4)}</h3>
                {:else if line.startsWith('- ')}
                  <p class="preview-li">• {line.slice(2)}</p>
                {:else if line.trim() === ''}
                  <br />
                {:else}
                  <p class="preview-p">{line}</p>
                {/if}
              {/each}
            </div>
          {:else}
            <textarea class="edit-textarea" bind:value={soulEditContent}></textarea>
          {/if}
        </div>
      {:else}
        <!-- Soul memory file cards -->
        <div class="soul-desc">
          <span class="soul-desc-text"
            >Human-editable markdown files injected into agent prompts. Version-controlled with git.</span
          >
        </div>
        <div class="soul-file-list">
          {#each SOUL_MEMORY_FILES as def}
            {@const file = soulMemoryStore.getFile(def.kind)}
            <div class="soul-card">
              <div class="soul-card-header">
                <div class="soul-card-title">
                  <span
                    class="soul-kind-badge"
                    class:active={file?.exists && file?.enabled}
                    class:disabled={file?.exists && !file?.enabled}
                  >
                    {def.label}
                  </span>
                  <span class="soul-filename">{def.fileName}</span>
                </div>
                {#if file?.exists}
                  <span class="soul-size">{formatSize(file.sizeBytes)}</span>
                {/if}
              </div>
              <div class="soul-card-desc">{def.description}</div>
              {#if file?.exists}
                <div class="soul-card-preview">
                  {file.content.slice(0, 120)}{file.content.length > 120 ? '...' : ''}
                </div>
                <div class="soul-card-actions">
                  <button class="btn-sm" onclick={() => startSoulEdit(def.kind)}>Edit Inline</button
                  >
                  <button class="btn-sm" onclick={() => openSoulFileInEditor(def.kind)}
                    >Open in Editor</button
                  >
                </div>
              {:else}
                <div class="soul-card-empty">
                  <button class="btn-sm primary" onclick={() => initSoulFile(def.kind)}>
                    Create {def.fileName}
                  </button>
                </div>
              {/if}
        ...
        {#if soulMemoryStore.state}
          <div class="soul-footer">
            <span class="soul-footer-text">
              Total injection: {formatSize(soulMemoryStore.state.totalInjectionSize)}
            </span>
          </div>
        {/if}
      {/if}
...
<style>
  ...
  .soul-kind-badge.active {
    background: var(--accent-primary);
    color: var(--bg-primary);
  }
  .soul-kind-badge.disabled {
    background: var(--bg-tertiary);
    color: var(--text-tertiary);
    opacity: 0.6;
  }
