...
function createUIStore() {
  ...
  let toasts = $state<Array<{ id: string; message: string; type: 'info' | 'success' | 'error' | 'warning'; timeout?: number }>>([]);
  ...
  return {
    ...
    setSidebarTab(tab: SidebarTab) { sidebarTab = tab; sidebarOpen = true; },
    setSidebarWidth(w: number) { sidebarWidth = Math.max(200, Math.min(500, w)); },

    openModal(id: ModalId) { activeModal = id; },
    closeModal() { activeModal = null; },
    openSettings() { activeModal = 'settings'; },
    openMcpManager() { activeModal = 'mcp-manager'; },

    toast(message: string, type: 'info' | 'success' | 'error' | 'warning' = 'info', timeout = 4000) {
    ...
    setCommandPaletteQuery(q: string) { commandPaletteQuery = q; },
  };
}
